...
#Finding the mean square error between two lists est and real
...
def popularity_estimator_idw(latitude,longitude,Train): 
	radius = 0.1 #miles
...
def popularity_estimator_idw_modified(latitude,longitude,Train): 
	radius = 0.1 #miles
...
def surcharge_estimator(Train,Test):
	...
	return metrics.mean_squared_error(y_1,targetTest)
...
def surcharge_estimator_local(Train,Test):
	...
	for index,row in featuresTest.iterrows():
		...
		while radius <= 1.0:
			...
			if len(poplist) == 0:
				...
				sur_e = lwr_predict_idw(features,target,datapoint)
				if sur_e < 1.0:
					y_1.append(1.0)
				else:
					y_1.append(sur_e)
	...
	#print "The MSE obtained  = "
	target = list(targetTest['surcharge'])
	return metrics.mean_squared_error(list(y_1),targetTest)
...
def main():
	...
	folding  = 0
	kf = KFold(total_items, n_folds=5)
	sur_mse = []
	pop_mse = []
	for train_index, test_index in kf:
		folding = folding + 1
		
		Train,Test = datainput.iloc[train_index],datainput.iloc[test_index]
		features = Train[["latitude","longitude","popularity"]]
		est_pop = []
		realpop = []
		for index, row in Test.iterrows():
			x =  popularity_estimator_idw(row['latitude'],row['longitude'],Train)
		...
		mse1 = surcharge_estimator_knn(features1,test_new)
		#print mse1
		sur_mse.append(mse1)
		
		#Printing MSE at each fold iteration and also current average error of surcharge
		#print "Mean popularity MSE:"
		#print pop_mse
		#print sum(pop_mse)/len(pop_mse)
		print "Mean Surcharge MSE"
		print sur_mse
		print sum(sur_mse)/len(sur_mse)
