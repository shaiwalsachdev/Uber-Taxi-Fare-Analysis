#Collect the UBER fare data for OD pairs which lie in the specified timezone 
#Importing Packages
import requests
import json
import sys
...
#Get the OD pair between startid and endid at the particular timezone

def getLocList(startid,endid,timezone):

    idlow = startid
    idhi = endid

    conread = sqlite3.connect("/home/bt2/14CS10061/shaiwal/Uber/ByTimezone/"+str(timezone)+"/nyc_taxi/zone"+str(timezone)+"_loc.sqlite")
...
def main():
	...
	loclist = getLocList(idlow,idhi,timezone)
	...
	outfilename = '/home/bt2/14CS10061/shaiwal/Uber/ByTimezone/'+str(timezone)+'/uber/'+str(timezone)+'uberprice_key'+str(keyid)+'_sid'+str(idlow)+'_eid'+str(idhi)+'.json.gz'
